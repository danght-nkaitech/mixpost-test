...
const gifProvider = computed(() => {
  return usePage().props.gif_provider
})

const enabled = computed(() => {
  return usePage().props.is_configured_service[gifProvider.value]
})
...
<template>
  <div v-if="enabled">
    <SearchInput v-model="keyword" :placeholder="$t(`service.${gifProvider}.search_gifs`)" />
  ...
  <template v-if="!enabled">
    <Alert variant="warning" :closeable="false">
      {{ $t('service.not_configured_service', { service: `${capitalize(gifProvider)}` }) }}
    </Alert>
