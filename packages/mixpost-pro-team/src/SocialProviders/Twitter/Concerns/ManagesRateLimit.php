...
trait ManagesRateLimit
{
    ...
    public function buildResponse($response, ?Closure $okResult = null): SocialProviderResponse
    {
        ...
        if (isset($response->status) && $response->status === 403) {
            Mixpost::reportException(new Exception(json_encode($response) ?: 'Twitter API 403 error'));
        ...
        Mixpost::reportException(new Exception(json_encode($response) ?: 'Twitter API error'));
